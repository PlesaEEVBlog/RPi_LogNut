...
while True:
    ...
    BME_B_Pressure = sensorB.read_pressure()
    BME_B_Humidity = sensorB.read_humidity()
    print time.strftime("%d/%m/%Y\t%H:%M:%S") + ("\033[1;32;40m\tTemperature: %2.2f C %2.2f C\tPressure: %6.0f Pa %6.0f Pa\tRelative Humidity: %3.1f %3.1f" % (float(BME_A_Temperature),float(BME_B_Temperature),float(BME_A_Pressure),float(BME_B_Pressure),float(BME_A_Humidity),float(BME_B_Humidity) ) )
    with open(LogFile, 'a') as EnvLog:
