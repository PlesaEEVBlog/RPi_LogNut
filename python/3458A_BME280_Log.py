...
LogDir = '/pub/logs/'
LogFile= LogDir + '3458A_BME280_Log.csv'
...
#3 reflevel = 7.106173685
#4 reflevel = 7.137473141
#1 reflevel = 7.16733881
reflevel = 7.16508192 
...
if (os.path.isfile(LogFile) == False):
  with open(LogFile, 'a') as MeterLog:
    if (BME_Enabled == 1):MeterLog.write("TimeStamp,Voltage,ppm,MeterTemperature,Temperature,Pressure,Humidity\r\n")
    if (BME_Enabled == 0):MeterLog.write("TimeStamp,Voltage,MeterTemperature,ppm\r\n")
    MeterLog.close()
      
# Check ACAL file presence, write header
if (os.path.isfile(LogFile_ACAL) == False):
  with open(LogFile_ACAL, 'a') as ACALLog:
    ACALLog.write("TimeStamp,DCV_Gain,MeterTemperature\r\n")
    ACALLog.close() 
...
while True:
  with open(LogFile, 'a') as MeterLog:
    ...
    if (tread == 0):
      ...
      inst.write("TEMP?")
      MeterTemperature = float(inst.read())
    if (abs(ACAL_Temperature - MeterTemperature) >= ACAL_Treshold) or (time.time() - Last_ACAL_TimeStamp >= ACAL_TimeInterval):
        print('MeterTemperature or time difference since last ACAL is higher than set treshold %.1f C / %d s') %(float(ACAL_Treshold),ACAL_TimeInterval)
        ...
        print "Internal Temperature = %2.1f" %float(MeterTemperature)
        ACAL_Temperature = MeterTemperature
        with open(LogFile_ACAL, 'a') as ACALLog:
          ACALLog.write (time.strftime("%d/%m/%Y-%H:%M:%S,") + ("%.9f,%.2f\r\n" % (float(DCV10Gain),float(MeterTemperature) ) ))
          ACALLog.close()
    inst.write("TARM SGL,1")
    data = inst.read()
    ...
    inst.write("DISP OFF,\"  \"")
    if (BME_Enabled == 1):
      print time.strftime("%d/%m/%Y\t%H:%M:%S") + ("\t%d x ACAL\t%.8f\t%.3f ppm\t%.1f C\tTemp: %2.2f C\tPres: %6.0f Pa\tHum: %3.1f" % (ACAL_Counter, float(data),float(ppm),float(MeterTemperature),float(Temperature),float(Pressure),float(Humidity) ) )
      MeterLog.write (time.strftime("%d/%m/%Y-%H:%M:%S,") + ("%.8f,%.1f,%.3f,%2.2f,%6.0f,%3.1f\r\n" % (float(data),float(MeterTemperature),float(ppm),float(Temperature),float(Pressure),float(Humidity) ) ))
    else:
      print time.strftime("%d/%m/%Y\t%H:%M:%S") + ("\t%d x ACAL\t%.8f\t%.3f ppm\t%.1f C" % (ACAL_Counter, float(data),float(ppm),float(MeterTemperature) ) )
      MeterLog.write (time.strftime("%d/%m/%Y-%H:%M:%S,") + ("%.8f,%.1f,%.3f\r\n" % (float(data),float(MeterTemperature),float(ppm) ) ))
    MeterLog.close()
